...
if (isset($_POST["depositar"])) {
    if (!empty($_POST["tipo"]) && !empty($_POST["documento"]) && !empty($_POST["telefono"]) && !empty($_POST["valor"]) &&!empty($_POST["codigo"])&& is_numeric($_POST["documento"]) && !preg_match("/[a-z]/",$_POST["documento"])&& 
    strlen ($_POST ["documento"])<=11 
    && is_numeric($_POST["telefono"]) && !preg_match("/[a-z]/",$_POST["telefono"])&& strlen ($_POST ["telefono"])<=10 && is_numeric($_POST["valor"]) && !preg_match("/[a-z]/",$_POST["valor"])&& strlen ($_POST ["valor"])<=100000000
    && is_numeric($_POST["codigo"]) && !preg_match("/[a-z]/",$_POST["codigo"])&& strlen ($_POST ["codigo"])<=6){
        $conexion =mysqli_connect("localhost","root","","lscinformation")
or die ("la conexion a la base de datos es fallida");
mysqli_query($conexion,"insert into depositos (detipodocumento,numdocumento,telefono,valor,dep_codigo) values ('$_REQUEST[tipo]','$_REQUEST[documento]','$_REQUEST[telefono]','$_REQUEST[valor]','$_REQUEST[codigo]')")
or die ("problemas de conexion".mysqli_error("$conexion"));
echo "deposito";

    }else { echo "No se ha hecho valido el deposito."."<br>"; 
 }   
