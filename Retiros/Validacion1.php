...
<?php
      if (isset($_POST["Retirar"])) {
          ...
          !empty($_POST["Valor"]) &&
          !empty($_POST["Codigo"])&& 
          is_numeric($_POST["Documento"]) && !preg_match("/[a-z]/",$_POST["Documento"])&& strlen ($_POST ["Documento"])<=11
          && is_numeric($_POST["Telefono"]) && !preg_match("/[a-z]/",$_POST["Telefono"])&& strlen ($_POST ["Telefono"])<=10 
          && is_numeric($_POST["Valor"]) && !preg_match("/[a-z]/",$_POST["Valor"])&& strlen ($_POST ["Valor"])<=100000000
      ...
      mysqli_query($conexion,"insert into retiros (Tipo_Documento,Numero_Documento,Numero_Telefono,Valor_Retiro,Codigo) 
      values ('$_REQUEST[Lista]','$_REQUEST[Documento]','$_REQUEST[Telefono]','$_REQUEST[Valor]','$_REQUEST[Codigo]')")
      or die ("problemas de conexion" .mysqli_error("$conexion"));
      mysqli_close($conexion);
      
      echo "Se ha realizado el retiro de manera exitosa ";
    
           }else { echo "No se ha generado el retiro, vuelva a intentar"."<br>"; 
       }  
